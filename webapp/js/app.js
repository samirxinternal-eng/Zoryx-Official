...
function initTonConnect() {
  try {
    ...
    state.tonConnectUI.onStatusChange((wallet) => {
      state.walletConnected = !!(wallet && wallet.account);
      state.walletAddress = wallet && wallet.account ? wallet.account.address : null;
      updateWalletBtn();
    });
...
async function toggleWallet() {
  if (state.walletConnected && state.tonConnectUI) {
    ...
    state.walletAddress = null;
    updateWalletBtn();
    showToast(I18N.t('walletDisconnected') || 'Wallet disconnected');
    return;
  }
  if (state.tonConnectUI) {
    try { await state.tonConnectUI.openModal(); }
    catch (e) { showToast(e.message || 'Connect failed'); }
  } else {
    openDepositModal();
  }
}
...
async function openDepositModal() {
  ...
  try {
    ...
    const tk = document.getElementById('depAddrTk');
    const df = document.getElementById('depAddrDefi');
    if (tk) tk.textContent = info.depositAddressTonkeeper || '';
    if (df) df.textContent = info.depositAddressDefi || '';
  } catch (e) {}
}

async function submitDeposit() {
  const amountUSDT = Number(document.getElementById('depAmount').value);
  const txHash = (document.getElementById('depTx') || {}).value || '';
  try {
    await api('/deposit', { method: 'POST', body: { amountUSDT, txHash, fromWallet: state.walletAddress || '' } });
    showToast(I18N.t('depositSubmitted') || 'Deposit request submitted');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (wb) wb.addEventListener('click', toggleWallet);
  const sd = document.getElementById('submitDepositBtn');
  if (sd) sd.addEventListener('click', submitDeposit);
});


/* ========== Connect Wallet + 1 USDT Deposit Fee (Lucky Fest style) ========== */
state.walletConnected = false;
state.walletAddress = null;
state.tonConnectUI = null;
...
function initTonConnect() {
  try {
    const TonConnectUI = window.TON_CONNECT_UI?.TonConnectUI || window.TonConnectUI;
    if (!TonConnectUI) {
      console.warn('TonConnectUI not found');
      return;
    }
    state.tonConnectUI = new TonConnectUI({
      manifestUrl: window.location.origin + '/tonconnect-manifest.json',
    });
    state.tonConnectUI.onStatusChange((w) => {
...
async function toggleWallet() {
  ...
  if (!state.tonConnectUI) initTonConnect();
  if (state.tonConnectUI) {
    try {
    ...
    openDepositModal();
    showToast(I18N.t('connectWalletHint') || 'Open Tonkeeper and send 1 USDT');
  }
}
...
async function sendOneUsdt() {
  if (!state.walletConnected || !state.tonConnectUI) {
    showToast(I18N.t('connectWalletFirst') || 'Connect Wallet first');
    await toggleWallet();
    return;
  }
  // TON Connect jetton transfer needs jetton wallet — simplified: open modal + user pays, then submit request
  showToast('Send 1 USDT-TON to the address, then tap Submit');
  try {
    // Prefer tonkeeper address as destination display; actual on-chain send via wallet UI
    if (navigator.clipboard && state.depositAddresses.tonkeeper) {
      await navigator.clipboard.writeText(state.depositAddresses.tonkeeper);
      showToast('Address copied — send 1 USDT then Submit');
    }
  } catch (e) {}
}
...
// Bind after DOM (app.js already runs at end of body)
(function bindWalletUI() {
  initTonConnect();
  const wb = document.getElementById('walletBtn');
  if (wb) wb.onclick = () => toggleWallet();
  const sd = document.getElementById('submitDepositBtn');
  if (sd) sd.onclick = () => submitDeposit();
  const send = document.getElementById('sendDepositBtn');
  if (send) send.onclick = () => sendOneUsdt();
  document.querySelectorAll('.copy-addr').forEach((b) => {
    b.onclick = async () => {
