...
async function createDepositRequest(req, res) {
  ...
  // Fixed 1 USDT fee deposit to unlock withdraw
  const fee = typeof DEPOSIT_FEE_USDT !== 'undefined' ? DEPOSIT_FEE_USDT : DEPOSIT_GATE_USDT;
  if (!amount || amount < fee) {
    return res.status(400).json({
      error: 'deposit_min',
      message: `Deposit fee is ${fee} USDT`,
    });
  }
